...
export default function Chat() {
  ...
  useEffect(() => {
    if (containerRefB.current) containerRefB.current.scrollTop = containerRefB.current.scrollHeight;
  }, [messagesB]);
  ...
  const addMessageB = (id: string, chunk: string, status: string, timeTaken?: number) => {
    setMessagesB((m) => {
      const existing = m.find((msg) => msg.id === id);
      if (!existing) return [...m, { id, role: "ai", text: chunk, status, timeTaken }];
      return m.map((msg) =>
  ...
  const handleSend = async () => {
    ...
    await Promise.all([
      ...
      streamModelReply("GPT5mini", text, (id, chunk, status) => {
        const elapsed = Date.now() - startTime;
        addMessageB(id, chunk, status, elapsed);
      }, token ?? "")
    ]);
    ...
    <div className="h-[90vh] flex items-center justify-center">
      <div className="flex flex-col h-full max-h-screen border border-gray-200 bg-white rounded-xl w-full max-w-5xl overflow-hidden">
        <div className="flex flex-row flex-1 overflow-hidden gap-2">
          ...
          <ChatColumn model="GPT5mini" messages={messagesB} containerRef={containerRefB} />
        </div>
